function [sim] = simulate_place_ego(param)
...
% calculate egocentric bearing at each timepoint
head_direction = get_hd(param.position);
rlX = param.ref_point(1,1); rlY = param.ref_point(1,2);
egoAng = mod((atan2d(rlY-ypos, rlX-xpos)+180) - head_direction, 360);
range = 10;
...
for frame = 1:length(t)
    ...
    if binX_now > 0 && binY_now > 0 && egoAng(frame) < param.theta + range && egoAng(frame) > param.theta - range
        ri = rand(1);
...
plot(param.ref_point(1,1), param.ref_point(1,2),'o','MarkerSize',10,...
    'MarkerEdgeColor','k',...
    'MarkerFaceColor','k');
...
set(gca,'xtick',[])
set(gca,'ytick',[])
title(rate_map_title, 'FontSize', 35)
box off
% 
% figure
...
% n = zeros(length(t),1);
% n_map = zeros(nBins,nBins);
% meanFR = zeros(length(t),1);
% for frame = 1:length(t)
%     binX_now = binX(frame); binY_now = binY(frame);
%     if binX_now > 0 && binY_now > 0  
%         occ_now = occ(binX_now, binY_now);
%         currentLambda = zMapped(binX_now, binY_now);
%         n(frame) = poissrnd(currentLambda); 
%         n_map(binX_now, binY_now) = n_map(binX_now, binY_now) + n(frame);
%         % calculate mean firing rate
%         meanFR(frame) = n(frame)/(occ_now*fs);
%     else
%         n(frame) = NaN;
%         meanFR(frame) = NaN;
%     end
...
% [spkPos] = data.getSpikePositions(spiketimes, param.position);
% dist_from_ctr = sqrt((xmu - spkPos(:,2)).^2+ (xmu - spkPos(:,3)).^2);
% max(dist_from_ctr)

% figure; imagesc(zMapped)
% hist(reshape(zMapped.*100, size(zMapped,1)*size(zMapped,2),1));
