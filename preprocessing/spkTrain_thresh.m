function [SpikeTrain_thresh] = spkTrain_thresh(position, SpikeTimes_thresh, SpikeTrain)
...
for sessNum = 1:length(position)
    if ~isempty(position{1,sessNum}) && ~isempty(SpikeTimes_thresh{1,sessNum})
        sessionArray = cell(1,length(SpikeTimes_thresh{1,sessNum}));
        for unitNum = 1:length(SpikeTrain{1,sessNum})
            ST_now = SpikeTimes_thresh{1,sessNum}{1,unitNum};
            SpikeTrain_thresh{1,sessNum} = binSpikes(position{1,sessNum}(:,1), SpikeTimes_thresh{1,sessNum});
        end
    end
end
