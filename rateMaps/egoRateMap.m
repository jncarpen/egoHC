...
DistanceToObject_y = y - ObjectPosition(2);
RToObject=sqrt(DistanceToObject_x.^2+DistanceToObject_y.^2);

% calculate angle to object
AngleToObject = rem(atan2d(y-ObjectPosition(2), x-ObjectPosition(1))+360, 360);
...
% create meshgrid
[t,RR] = meshgrid(linspace(0,2*pi,round(360/AngleBin)+1),linspace(0,maxSpaceBin,SpaceBin+1));
ECOMap = zeros(SpaceBin+1,round(360/AngleBin)+1);
ECOFiringMap = zeros(SpaceBin+1,round(360/AngleBin)+1);
...
[X,Y] = pol2cart(t,RR); % i dont think this line of code is actually being used.
ECOFiringMap_normalized = zeros(length(SpaceBin), length(1:1:round(360/AngleBin))); % pre-allocate size
% normalize the firing map
for i=1:1:SpaceBin
    for jj=1:1:round(360/AngleBin)
        if ECOMap(i,jj)>0
            ECOFiringMap_normalized(i,jj)=ECOFiringMap(i,jj)./ECOMap(i,jj);
        else
        end
    end
end
...
%% smooth maps
sigma = 2;
ECOMap_smoothed=imgaussfilt(ECOMap,sigma);
ECOFiringMap_smoothed=imgaussfilt(ECOFiringMap,sigma);
ECOFiringMap_normalized_smoothed=imgaussfilt(ECOFiringMap_normalized,sigma);
...
if nargin < 1
   [tt,rr] = meshgrid(linspace(0,2*pi,361),linspace(-4,4,101));
   [xx,yy] = pol2cart(tt,rr);
   peakpeak = peaks(xx,yy);                       % peaks function on a polar grid
                                         % draw 3d polar plot
   figure('Color','white','NumberTitle','off','Name','PolarPlot3d v4.3');
   polarplot3d(peakpeak,'PlotType','surfn','PolarGrid',{4 24},'TickSpacing',8,...
                 'AngularRange',[30 270]*pi/180,'RadialRange',[.8 4],...
                 'RadLabels',3,'RadLabelLocation',{180 'max'},'RadLabelColor','red');
                                         % set plot attributes
   set(gca,'DataAspectRatio',[1 1 10],'View',[-12,38],...
...
if ~isequal(p.axislocation,'off')
   ...
   if ~isempty(p.tickspacing) && p.tickspacing > 0 && p.tickspacing <= 180
      ...
      % Add tick labels
      for ik = 2 * (1:nl) - 1
         text(xt(3,ik),yt(3,ik),zt(ik),num2str(ta(ik)*180/pi),...
...
if p.radlabels > 0 && ~isequal(p.radlabellocation,'off')
   ...
   hold on;
   for ik = 1:length(tr)
      text(xt(ik),yt(ik),zt(ik),num2str(tr(ik)),...
...
if r > 1 || m > 1
   ...
   for ik = 1:r-1
      rho((ik-1)*u+1:(ik*u)+1) = linspace(radgrid(ik),radgrid(ik+1),u+1);
   end
   for ik = 1:m-1
      angl((ik-1)*v+1:(ik*v)+1) = linspace(azmgrid(ik),azmgrid(ik+1),v+1);
   end
...
for ik = 1:length(fname)
   ...
   if isfield(S,fname{ik})
      % Is this a substructure ?
      if isstruct(T.(fname{ik})) && isstruct(S.(fname{ik}))
         % Recursively process the substructure
         T.(fname{ik}) = structrecon(S.(fname{ik}),T.(fname{ik}));
      % Not a substructure, copy field value from S
      else T.(fname{ik}) = S.(fname{ik});
      end
      S = rmfield(S,fname{ik});
   end
end
end
...
for ik = 1:2:length(varargin)
   S.(lower(varargin{ik})) = varargin{ik+1};
end
end
...
function m = ismono(v)
[rx,c] = size(v);       % size of input
if rx == 1,             % row vector
   v = v';             %   transpose
   rx = c; c = 1;       %   size
end
sgn   = sum(sign(diff(v,1,1)),1);  % sgn is r+1 or -(r+1) for monotonic columns
up    = sgn - repmat(rx,1,c);       % subtract r to detect increasing (-1)
down  = sgn + repmat(rx,1,c);       % add      r to detect decreasing (+1)
