function plot_cosineFit(model, P, ST)
%PLOT_COSINEFIT

best = model.bestParams;
X = model.loc3d(:,:,1); 
Y = model.loc3d(:,:,2);
...
for h = 1:length(model.bins)
    H(:,:,h) = ones(10,10).*model.bins(h);
end
...
for h = 1:10
    real_tc(h) = nanmean(reshape(model.data(:,:,h), 100, 1));
...
plt_real = plot(deg2rad(model.bins-180), real_tc,'ko','MarkerFaceColor','b');
